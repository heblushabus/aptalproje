...
#define TOUCH_BUTTON_5_CHAN_ID 5
...
static bool touch_on_active_callback(touch_sensor_handle_t sens_handle,
  ...
  } else if (event->chan_id == TOUCH_BUTTON_5_CHAN_ID) {
    status.touch_5 = true;
    ESP_EARLY_LOGI(TAG, "Touch 5 Active");
  }
...
static bool touch_on_inactive_callback(touch_sensor_handle_t sens_handle,
  ...
  if (event->chan_id == TOUCH_BUTTON_4_CHAN_ID) {
    ...
    status.touch_5 = false;
    ESP_EARLY_LOGI(TAG, "Touch 5 Inactive");
  }
  global_data.setStatus(status);
  return false;
}
...
esp_err_t TouchManager::init() {
  ...
  ESP_ERROR_CHECK(touch_sensor_new_channel(sens_handle, TOUCH_BUTTON_5_CHAN_ID,
                                           &chan_cfg, &chan_handle_5));
  ...
  touch_sensor_get_channel_info(chan_handle_5, &chan_info);
  ESP_LOGI(TAG, "Touch Chan 5 mapped to GPIO %d", chan_info.chan_gpio);
  ...
  uint32_t smooth[1] = {0};
  touch_channel_handle_t handles[] = {chan_handle_4, chan_handle_5};
  int ids[] = {TOUCH_BUTTON_4_CHAN_ID, TOUCH_BUTTON_5_CHAN_ID};

  for (int i = 0; i < 2; i++) {
    // Read SMOOTH data first as it reflects current capacitance
    ESP_ERROR_CHECK(touch_channel_read_data(
        handles[i], TOUCH_CHAN_DATA_TYPE_SMOOTH, smooth));
    ESP_ERROR_CHECK(touch_channel_read_data(
        handles[i], TOUCH_CHAN_DATA_TYPE_BENCHMARK, benchmark));
    ...
    uint32_t thresh = (uint32_t)(benchmark_val * 0.02);
    if (thresh < 500)
      thresh = 500; // Minimum threshold floor

    update_cfg.active_thresh[0] = thresh;

    ESP_ERROR_CHECK(touch_sensor_reconfig_channel(handles[i], &update_cfg));

    ESP_LOGI(TAG, "Touch %d Smooth: %lu, Benchmark: %lu, Set Threshold: %lu",
             ids[i], smooth[0], benchmark[0], update_cfg.active_thresh[0]);
  }
